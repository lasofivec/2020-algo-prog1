...
#----
# taux est un flottant compris entre 0 et 1.
# Si taux est inferieur a 0 alors on considere qu'il est nul
# Si taux est superieur a 1, alors on considere qu'il est egal a 1
# Cette fonction detecte les impasses (un seul voisin relie)
# Sur (100 * taux)% des impasses, on choisit un voisin au hasard
# qui n'est pas relie a cette impasse et on l'ouvre.

	def ouvrirImpasses(self,taux):
		for nx in range(self.largeur):
			for ny in range(self.hauteur):
				c = self.cellules[nx][ny]
				if(len(self.tab_voisins[nx][ny].relies)==1): # si impasse
					if(random.random()<taux):
						c = self.cellules[nx][ny]
						vois_relie = self.tab_voisins[nx][ny].relies[0]
						voisins = self.tab_voisins[nx][ny].simples
						for v in voisins :
							if(v.estEgalA(vois_relie)):
								voisins.remove(v)
						rnd = int(len(voisins)*random.random())
						vois_a_joindre = voisins[rnd]
						self.relier(c,vois_a_joindre)
...
#----
	...
	def tableau2D(self):
		...
		# Tout d'abord faire le canevas communs à tous les labyrinthes (points aux points impairs)

		for nx in range(self.mtx_largeur):
			if nx==self.mtx_largeur-1 and not estImpair(self.mtx_largeur):
				colonne = [0]*self.mtx_hauteur
			else:
				colonne = []
				for ny in range(self.mtx_hauteur):
					if ny == self.mtx_hauteur-1 and not estImpair(self.mtx_hauteur):
						colonne += [0]
					elif ( estImpair(nx) and estImpair(ny) ):
						colonne += [1]
					else :
						colonne += [0]
